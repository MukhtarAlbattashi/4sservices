    ...
    <div class="container-fluid px-5">
        <div class="card">
            <div class="card-header">
                <h4>{{ __("public.categoriesAllowances") }} - {{$allowances->total()}}</h4>
            </div>
            <div class="card-body">
                <div class="row justify-content-between">
                    <div class="col-md-3">
                        @can(\App\Enums\AppPermissions::CAN_CREATE_ALLOWANCE_CATEGORIES)
                            <button class="btn btn-dark m-2 m-2" wire:click="add"
                            >
                                {{ __('public.add') }} {{ __("public.categoriesAllowances") }}
                            </button>
                        @endcan
                    </div>
                    <div class="col-md-3">
                        ...
                        <table class="table   table-bordered table-sm">
                            ...
                            @foreach($allowances as $allowance)
                                <tr class="text-center table-font align-middle">
                                    ...
                                    <td>{{app()->getLocale()=='ar' ? $allowance->arName : $allowance->enName}}</td>
                                    <td>
                                        {{$allowance->allowances_count}}
                                    </td>
                                    <td>
                                        {{date('Y-m-d', strtotime($allowance->created_at))}}
                                    </td>
                                    <td>
                                        @can(\App\Enums\AppPermissions::CAN_EDIT_ALLOWANCE_CATEGORIES)
                                            <button
                                                wire:click="update({{$allowance->id}})"
                                                class="btn btn-outline-primary btn-sm rounded-3">
                                                <span class="fas fa-edit"></span>
                                                {{__('public.edit')}}
                                            </button>
                                        @endcan
                                        @can(\App\Enums\AppPermissions::CAN_DELETE_ALLOWANCE_CATEGORIES)
                                            <button
