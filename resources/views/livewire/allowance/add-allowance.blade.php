            ...
            <div class="card-body">
                <div class="row  border-bottom p-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input wire:model.defer="allowance.name" type="text" class="form-control text-center" id="name"
                               required>
                        @error('allowance.name') <span class="error text-danger fs-6">{{ $message }}</span> @enderror
                    </div>
                    <div class="col-md-3">
                        <label for="amount" class="form-label mt-2">{{ __("public.amount")}}</label>
                        <input wire:model.defer="allowance.amount" type="number" class="form-control text-center"
                               id="amount" required>
                        @error('allowance.amount') <span class="error text-danger fs-6">{{ $message }}</span> @enderror
                    </div>
                    <div class="col-md-3">
                        <label for="date" class="form-label mt-2">{{ __("public.date")}}</label>
                        <input wire:model.defer="allowance.date" type="date" class="form-control text-center" id="date"
                               required>
                        @error('allowance.date') <span class="error text-danger fs-6">{{ $message }}</span> @enderror
                    </div>
                    <div class="col-md-12">
                        <label for="notes" class="form-label mt-2">{{ __("public.notes")}}</label>
                        <textarea wire:model.defer="allowance.notes" type="text" class="form-control text-center"
                                  id="notes" required></textarea>
                        @error('allowance.notes') <span class="error text-danger fs-6">{{ $message }}</span> @enderror
                    </div>
                </div>
