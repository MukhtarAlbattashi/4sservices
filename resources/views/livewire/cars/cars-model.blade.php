                ...
                <div class="row justify-content-between">
                    ...
                    <div class="col-md-12 mt-3 table-responsive" wire:ignore.self>
                        <table class="table  table-bordered table-sm table-responsive">
                            ...
                            <td>{{ __("public.carBrand") }}</td>
                            <td>{{ __("public.carModel") }}</td>
                            ...
                            @foreach($cars as $car)
                                <tr class="text-center table-font align-middle">
                                    ...
                                    <td class="d-flex justify-content-center gap-3">
                                        @can(\App\Enums\AppPermissions::CAN_EDIT_CAR_MODELS)
                                            <button
                                        ...
                                        @endcan
                                        @can(\App\Enums\AppPermissions::CAN_DELETE_CAR_MODELS)
                                            <button wire:loading.attr="disabled"
                                                    x-data
                                                    @click="$wire.set('CarModelId', {{$car->id}}, true); $dispatch('show-delete-model')"
            ...
            <div class="container-fluid my-3">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <select name="brand" wire:model.defer="brand" class="form-select" id="brand">
                    ...
                    <div class="col-md-12 mt-3">
                        <button class="btn btn-success" wire:click="save" wire:loading.attr="disabled">{{__('public.save')}}</button>
                    </div>
                </div>
            </div>
        </x-modal>
        ...
        <x-modal id="dangerModal" title="public.delete" background="bg-danger">
            <div class="text-center">
                ...
                <button class="btn btn-danger text-white" wire:click="delete"
                        type="button" wire:loading.attr="disabled">{{__('public.delete')}}</button>
            </div>
        </x-modal>
        ...
        <x-modal id="editModal" title="public.edit" background="bg-primary">
            <div class="container-fluid my-3">
                <div class="row">
                    <div class="col-md-4">
                        <label for="brand" class="form-label mt-2">{{ __("public.carBrand")}}*</label>
                            <select name="brand" wire:model.defer="brand" class="form-select">
                                <option value="">{{__('public.choose')}}</option>
                                @foreach($this->getOptions() as $id => $name)
                                    <option value="{{ $id }}">{{ $name }}</option>
                                @endforeach
