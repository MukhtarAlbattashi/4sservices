    ...
    <div class="container-fluid px-5">
        <div class="card">
            <div class="card-header">
                <h4>{{ __("public.categoriesDiscounts") }} - {{$expenses->total()}}</h4>
            </div>
            <div class="card-body">
                <div class="row justify-content-between">
                    <div class="col-md-3">
                        @can(\App\Enums\AppPermissions::CAN_CREATE_DISCOUNT_CATEGORIES)
                            <button class="btn btn-dark m-2" wire:click="add"
                            >
                                {{ __('public.add') }} {{ __("public.categoriesDiscounts") }}
                            </button>
                        @endcan
                    </div>
                    <div class="col-md-3">
                        ...
                        <table class="table   table-bordered table-sm">
                            ...
                            @foreach($expenses as $expense)
                                <tr class="text-center table-font align-middle">
                                    ...
                                    <td>
                                        {{$expense->discounts_count}}
                                    </td>
                                    <td>
                                        {{date('Y-m-d', strtotime($expense->created_at))}}
                                    </td>
                                    <td>
                                        @can(\App\Enums\AppPermissions::CAN_EDIT_DISCOUNT_CATEGORIES)
                                            <button
                                        ...
                                        @endcan
                                        @can(\App\Enums\AppPermissions::CAN_DELETE_DISCOUNT_CATEGORIES)
                                            <button
