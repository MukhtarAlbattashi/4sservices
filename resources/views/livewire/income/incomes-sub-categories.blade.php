    ...
    <div class="container-fluid px-5">
        <div class="card">
            <div class="card-header">
                <h4>{{ __('public.incomeSubCategories') }} - {{$expenses->total()}}</h4>
            </div>
            <div class="card-body">
                <div class="row justify-content-between">
                    <div class="col-md-3">
                        @can(\App\Enums\AppPermissions::CAN_CREATE_INCOME_SUBCATEGORIES)
                            <button class="btn btn-dark m-2" wire:click="add"
                            >
                                {{ __('public.add') }} {{ __('public.incomeSubCategories') }}
                            </button>
                        @endcan
                    </div>
                    <div class="col-md-3">
                        ...
                        <table class="table   table-bordered table-sm">
                            ...
                            @foreach($expenses as $expense)
                                <tr class="text-center table-font align-middle">
                                    ...
                                    <td>
                                        @can(\App\Enums\AppPermissions::CAN_EDIT_INCOME_SUBCATEGORIES)
                                            <button
                                        ...
                                        @endcan
                                        @can(\App\Enums\AppPermissions::CAN_DELETE_INCOME_SUBCATEGORIES)
                                            <button
        ...
        <x-modal id="createModal" title="public.add" background="bg-dark">
            <div class="container-fluid my-3">
                <div class="row">
                    <div class="col-md-12">
                        <label for="category" class="form-label mt-2">{{ __("public.incomeCategories")}}
                            *</label>
        ...
        <x-modal id="editModal" title="public.edit" background="bg-primary">
            <div class="container-fluid my-3">
                <div class="row">
                    <div class="col-md-12">
                        <label for="category" class="form-label mt-2">{{ __("public.expenseCategories")}}
                            *</label>
                        <select name="category" wire:model.defer="category" class="form-select">
                            <option value="">{{__('public.choose')}}</option>
                            @foreach($this->getOptions() as $id => $name)
