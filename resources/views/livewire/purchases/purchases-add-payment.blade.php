                    ...
                    <div class="col-md-12">
                        <table class="table table-bordered table-responsive table-sm">
                            ...
                            <tr>
                                ...
                                <td>{{$purchasePayment->supplier->phone}}</td>
                            </tr>
                            <tr>
                                <td>{{ __("public.invoiceNumber") }}</td>
                    ...
                    <div>
                        <h4 class="text-center">
                            <span class="text-primary">({{__('public.totalAmount')}} :
                                {{number_format($purchasePayment->total,3)}} ) - </span>
                            <span class="text-success">({{__('public.paid')}} : {{number_format($paid,3)}} ) = </span>
                            <span class="text-danger">({{__('public.restAmount')}} :
                                {{number_format($purchasePayment->total - $paid,3)}} )</span>
                        </h4>
                    </div>
                    <hr class="bg-danger border-2 border-top border-dark mt-2">
                    <div class="col-md-12">
                        <h5 class="text-center text-danger">{{__('public.details')}} {{__('public.paymentsPurchases')}}
                        </h5>
                    </div>
                    <div class="col-md-12">
                        <table class="table   table-bordered table-sm">
                            ...
                            @foreach($purchasePayment->payments as $payment)
                                <tr class=" text-center align-middle" :wire:key="{{$payment->id}}">
                                    ...
                                    <td>
                                        @can(\App\Enums\AppPermissions::CAN_SHOW_PURCHASE_PAYMENTS)
                                            <a href="{{route('purchases-print-payment',$payment->id)}}"
