    ...
    <div class="container-fluid px-5">
        <div class="card">
            <div class="card-header">
                <h4>{{ __("public.paymentMethods") }} - {{$cars->total()}}</h4>
            </div>
            <div class="card-body">
                <div class="row justify-content-between">
                    <div class="col-md-3">
                        @can(\App\Enums\AppPermissions::CAN_CREATE_PAYMENTS)
                            <button class="btn btn-dark m-2" wire:click="add"
                            >
                                {{ __('public.add') }} {{ __("public.paymentMethods") }}
                            </button>
                        @endcan
                    </div>
                    <div class="col-md-3">
                        ...
                        <table class="table   table-bordered table-sm">
                            ...
                            @foreach($cars as $car)
                                <tr class="text-center table-font align-middle">
                                    ...
                                    <td>
                                        @can(\App\Enums\AppPermissions::CAN_EDIT_PAYMENTS)
                                            <button
                                        ...
                                        @endcan
                                        @can(\App\Enums\AppPermissions::CAN_DELETE_PAYMENTS)
                                            <button
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 mt-3">
                        <button class="btn btn-primary"
                                wire:click="updateCar">{{__('public.save')}}</button>
                    </div>
                </div>
            </div>
        </x-modal>
