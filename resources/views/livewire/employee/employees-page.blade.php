    ...
    <div class="container-fluid px-5">
        <div class="card">
            <div class="card-header">
                <h4>{{ __("public.employees") }} - {{$users->total()}}</h4>
            </div>
            <div class="card-body">
                <div class="row justify-content-between">
                    <div class="col-md-3">
                        @can(\App\Enums\AppPermissions::CAN_CREATE_EMPLOYEES)
                            <a class="btn btn-dark m-2" href="{{route('employee-create')}}">
                                {{ __('public.add') }} {{ __('public.employees') }}
                            </a>
                        @endcan
                    </div>
                    <div class="col-md-3">
                        ...
                        <table class="table   table-bordered table-sm">
                            ...
                            @foreach($users as $user)
                                <tr class="text-center table-font align-middle">
                                    ...
                                    <td>
                                        {{app()->getLocale()=='ar' ? $user->employeeNameAr : $user->employeeNameEn}}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @can(\App\Enums\AppPermissions::CAN_SHOW_EMPLOYEES)
                                            <a href="{{route('view-employee',$user)}}"
                                        ...
                                        @can(\App\Enums\AppPermissions::CAN_EDIT_EMPLOYEES)
                                            <a href="{{route('edit-employee',$user)}}"
                                               class="btn btn-outline-primary btn-sm rounded-3">
                                                <span class="fas fa-edit"></span>
                                                {{__('public.edit')}}
                                            </a>
                                        @endcan
                                        @can(\App\Enums\AppPermissions::CAN_DELETE_EMPLOYEES)
                                            <button
