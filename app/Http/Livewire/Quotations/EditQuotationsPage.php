...
class EditQuotationsPage extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function mount($id)
    {
        ...
        $this->tax = $this->gtax;
        $this->quotation = Quotation::query()->find($id);
        $this->car = $this->quotation->car;
        foreach ($this->quotation->parts as $part) {
            $part->makeHidden('pivot');
            $this->addedParts[] = $part->toArray();
            $this->quantity[] = $part->pivot->quantity;
        }
        foreach ($this->quotation->services as $service) {
    ...
    public function render()
    {
        ...
        return view('livewire.quotations.edit-quotations-page', [
            'parts' => Part::query()
    ...
    {
        $quotation = Quotation::query()->find($this->quotation->id);
        ...
        $quotation->save();
        $quotation->parts()->detach();
        $quotation->services()->detach();
